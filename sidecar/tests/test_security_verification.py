...
class TestFilesystemViaGateway:
    ...
    def test_write_blocked_by_view(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("view")
        resp = client.post("/v1/execute", json={"tool_id": "filesystem.write", "params": {"path": "C:\\test_blocked.txt", "content": "x"}})
    ...
    def test_write_allowed_at_admin(self):
        ...
        try:
            resp = client.post("/v1/execute", json={"tool_id": "filesystem.write", "params": {"path": test_file, "content": "sec test"}})
            assert resp.status_code == 200
...
class TestExecutorViaGateway:
    def test_command_blocked_by_view(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("view")
        resp = client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "echo blocked", "timeout": 5}})
        _blocked(resp, "view should block executor.command")

    def test_command_allowed_at_admin(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("admin")
        resp = client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "echo test123", "timeout": 10}})
        assert resp.status_code == 200
        assert resp.json()["success"] == True
...
class TestPolicyIntegration:
    def test_emergency_stop_blocks_filesystem_write(self):
        ...
        resp = client.post("/v1/execute", json={"tool_id": "filesystem.write", "params": {"path": "C:\\test_estop.txt", "content": "x"}})
    ...
    def test_pending_action_id_generated(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("confirm")
        resp = client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "rm dangerous_file", "timeout": 10}})
        assert resp.status_code == 200
        assert resp.json()["requires_confirmation"] == True
...
class TestAuditTrail:
    def test_audit_logs_filesystem_write(self):
        ...
        try:
            resp = client.post("/v1/execute", json={"tool_id": "filesystem.write", "params": {"path": test_file, "content": "audit me"}})
            assert resp.status_code == 200
        finally:
            if os.path.exists(test_file):
                os.remove(test_file)
    ...
    def test_audit_logs_executor_command(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("admin")
        client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "echo audit_test_cmd", "timeout": 5}})
        resp = client.get("/v1/audit")
        entries = resp.json().get("entries", [])
    ...
    def test_audit_logs_pending_confirmation(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("confirm")
        resp = client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "format sec_test", "timeout": 10}})
        assert resp.json()["requires_confirmation"] == True

    def test_audit_logs_policy_blocked(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("view")
        resp = client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "echo blocked_audit", "timeout": 5}})
        assert resp.json()["success"] == False

    def test_audit_log_entry_has_timestamp(self):
        from modules.permissions import _svc as perm_svc
        perm_svc.set_level("admin")
        client.post("/v1/execute", json={"tool_id": "executor.command", "params": {"command": "echo ts_test", "timeout": 5}})
        resp = client.get("/v1/audit")
        entries = resp.json().get("entries", [])
...
class TestNoBypass:
    ...
    def test_no_unknown_router_direct_service_calls(self):
        ...
        for mod, name in [(fs_mod, "filesystem"), (exec_mod, "executor"),
                          (sb_mod, "sentinel_bridge"), (pro_mod, "proactive")]:
    ...
    def test_gateway_has_all_expected_tools(self):
        ...
        expected = {
            ...
            "system.info", "system.cpu", "system.memory", "system.disk", "system.network", "system.processes", "system.gpu",
            "app.discovery",
            "ai.chat", "ai.analyze", "ai.config",
            "fleet.status", "fleet.generate_pairing", "fleet.revoke_pairing", "fleet.toggle_remote", "fleet.qr",
            "plugins.list", "plugins.templates", "plugins.load", "plugins.unload", "plugins.reload", "plugins.toggle", "plugins.create",
            "permissions.status", "permissions.set_level", "permissions.emergency", "permissions.confirm",
            "audit.list",
            "proactive.suggestions", "proactive.dismiss", "proactive.trend",
            "trigger.list", "trigger.create", "trigger.delete", "trigger.history", "trigger.evaluate",
        }
