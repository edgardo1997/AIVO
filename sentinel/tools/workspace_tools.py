...
class WorkspaceListTool(Tool):
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        return ToolResult.ok(
            data={"workspaces": self._svc.list_workspaces(), "active": self._svc.status()["active"]},
...
class WorkspaceCreateTool(Tool):
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        return ToolResult.ok(
            data=self._svc.create(params["workspace_id"], path=params.get("path", ""), session_id=sid, request_id=rid),
            tool_id="workspace.create",
        )
...
class WorkspaceOpenTool(Tool):
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        rid = (context or {}).get("request_id", "")
        return ToolResult.ok(
            data=self._svc.open(params["workspace_id"], session_id=sid, request_id=rid), tool_id="workspace.open"
        )
...
class WorkspaceDeleteTool(Tool):
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        rid = (context or {}).get("request_id", "")
        return ToolResult.ok(
            data=self._svc.delete(params["workspace_id"], session_id=sid, request_id=rid), tool_id="workspace.delete"
        )
