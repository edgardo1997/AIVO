"""Isolation audit and operational events."""
...
def isolation_events(
    result: IsolationContextResultV1,
    *,
    valid_origin: bool,
) -> tuple[AuditEventV1, OperationalEventV1]:
    audit = AuditEventV1(
        event_id=f"audit:{result.isolation_id}",
        event_type="V2_RUNTIME_ISOLATION_EVALUATED",
    ...
    operational = OperationalEventV1(
        event_id=f"telemetry:{result.isolation_id}",
        correlation_id=result.correlation_id,
        evidence_hash=result.evidence_hash,
        issuer_id=result.issuer_id,
        timestamp=result.timestamp,
