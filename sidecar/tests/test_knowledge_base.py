...
class TestVectorStore:
    ...
    def test_add_and_search(self, store):
        ...
        c1 = Chunk(chunk_id="c1", doc_id="d1", text="apple fruit",
                   embedding=[1.0, 0.0, 0.0], created_at="now")
        c2 = Chunk(chunk_id="c2", doc_id="d1", text="quantum physics",
                   embedding=[0.0, 1.0, 0.0], created_at="now")
    ...
    def test_search_returns_k_results(self, store):
        from sentinel.core.knowledge_base import Chunk
        chunks = [
            Chunk(chunk_id=f"c{i}", doc_id="d1", text=f"text {i}",
                  embedding=[float(i), 0.0, 0.0], created_at="now")
            for i in range(10)
        ]
    ...
    def test_list_docs(self, store):
        from sentinel.core.knowledge_base import Chunk
        store.add_many([
            Chunk(chunk_id="c1", doc_id="d1", text="a", embedding=[0.0],
                  metadata={"source": "src1"}, created_at="now"),
            Chunk(chunk_id="c2", doc_id="d1", text="b", embedding=[0.0],
                  metadata={"source": "src1"}, created_at="now"),
            Chunk(chunk_id="c3", doc_id="d2", text="c", embedding=[0.0],
                  metadata={"source": "src2"}, created_at="now"),
        ])
    ...
    def test_save_and_load(self, store):
        from sentinel.core.knowledge_base import Chunk
        store.add_many([
            Chunk(chunk_id="c1", doc_id="d1", text="hello",
