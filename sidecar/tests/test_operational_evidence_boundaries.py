...
MODULE = ROOT / "sentinel" / "v2_operational_evidence_storage"
FORBIDDEN_IMPORTS = {
    "subprocess",
...
def test_existing_runtime_does_not_import_storage() -> None:
    offenders = []
    for folder in (ROOT / "sentinel" / "core", ROOT / "sidecar" / "services"):
        if folder.exists():
            for path in folder.rglob("*.py"):
                if "sentinel.v2_operational_evidence_storage" in path.read_text(
                    encoding="utf-8",
                    errors="ignore",
                ):
