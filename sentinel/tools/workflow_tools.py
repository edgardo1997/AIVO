...
class WorkflowCreateTool(Tool):
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            ...
            description="Create a new AI workflow with ordered steps",
            version="1.0.0",
            category=_CAT,
            parameters={
                "type": "object",
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        return ToolResult.ok(
            data=self._svc.create(params["name"], params.get("steps", []), session_id=sid, request_id=rid),
            tool_id="workflow.create",
        )
...
class WorkflowExecuteTool(Tool):
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            ...
            description="Start or advance a workflow execution",
            version="1.0.0",
            category=_CAT,
            parameters={
                "type": "object",
                "properties": {
                    ...
                    "action": {
                        "type": "string",
                        "enum": ["start", "step", "complete", "fail"],
                        "description": "Action: start, step, complete, or fail",
                    },
                    "step_result": {"type": "string", "description": "Result data for a step execution"},
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        if action == "start":
            ...
            data = self._svc.execute_step(
                wid, step_result=params.get("step_result", ""), session_id=sid, request_id=rid
            )
