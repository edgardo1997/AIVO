...
- Live clarification flow: **FUNCTIONAL IN SOURCE** — durable `ClarificationRecord` store in `repositories/clarification_store.py`; `ClarificationService` with create/resolve/cancel/supersede; authenticated `POST /api/sentinel/clarifications/{id}/{resolve,cancel}` endpoints; `sentinel_bridge.py` emits a structured `clarification` NDJSON stream event and pauses the pipeline; 13 `alpha_constitutional_gate` tests in `tests/test_live_clarification_flow.py` pass; `Workbench` renders `Clarification` on a live stream event, submits option/free-text/cancel to the backend, and resends the resolved utterance; `Clarification.test.tsx` (5 tests), `npm test` (151 tests), `npm run build` and `cargo check` green.
- Compiled runtime smoke: **LABORATORY PARTIAL** — `pyinstaller sidecar.spec` produced `sidecar/dist/sidecar.exe` (82 MB) and `cargo build --release` produced `src-tauri/target/release/sentinel.exe` (14 MB) and bundled the sidecar at `src-tauri/target/release/sidecar/sidecar.exe`; the compiled sidecar launched, bound `127.0.0.1:8765`, passed `/api/health`, `/api/sentinel/conversation/capabilities` and `/api/sentinel/chat` with the session token; no GUI launch, no clean-VM install, no real PDF demo.
