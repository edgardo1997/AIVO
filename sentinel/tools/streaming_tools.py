...
class StreamingStatusTool(Tool):
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            id="streaming.status",
            name="Streaming Mode Status",
            description="Current streaming mode status",
            version="1.0.0",
            category=_CAT,
...
class StreamingActivateTool(Tool):
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            id="streaming.activate",
            name="Activate Streaming Mode",
            description="Activate streaming mode for a platform",
            version="1.0.0",
            category=_CAT,
            parameters={
                "type": "object",
                "properties": {
                    "platform": {"type": "string", "description": "Streaming platform (twitch, youtube, etc)"}
                },
                "required": [],
            },
    ...
    async def execute(self, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        return ToolResult.ok(
            data=self._svc.activate(platform=params.get("platform", ""), session_id=sid, request_id=rid),
            tool_id="streaming.activate",
        )
...
class StreamingDeactivateTool(Tool):
    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            id="streaming.deactivate",
            name="Deactivate Streaming Mode",
            description="Deactivate streaming mode",
            version="1.0.0",
            category=_CAT,
            parameters={"type": "object", "properties": {}, "required": []},
            required_permissions=["system.write"],
